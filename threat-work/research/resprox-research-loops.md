...
**Created:** 2026-09-08  
**Readiness:** C05/C01/C06 specified for Sentinel / Log Analytics with unconfirmed ingestion and L04 on-paper synthetic/triage sheets; no query or synthetic execution, validated detection or production change.
...
**Programme status:** in progress; L01–L04 completed  
**Next pass:** L05 / T04 — Endpoint and process hunts  
**Last completed pass:** L04  
**Last result journal entry:** [L04 — Synthetic controls, triage and rejected discrimination claims](#l04--synthetic-controls-triage-and-rejected-discrimination-claims)  
**Unresolved execution blocker:** none; L04 fixture construction, hand calculation and independent review completed; no test evaluator was run.  
...
L04 adds on-paper controls and triage to C05/C01/C06, narrows C01 ranking and corrects evidence/measurement handling. No hypothesis has been empirically validated. Earlier findings remain historical; L04 corrections supersede differing L03 details.
...
L01–L04 are recorded below. Append later passes without replacing earlier findings.
...
**Next pending pass: L05 / T04 — Develop endpoint and process hunts.**

L01–L04 are complete. Read the L03 specifications together with the superseding L04 amendments/test sheets, H00–H11 and S01–S37 plus A04. The first portfolio remains C05/C01/C06. L04 constructed on-paper fixtures, hand-calculated results and triage guidance; **no fixture evaluator, KQL/parser or tenant query has run**.

- Advance a durable endpoint/provenance or networking-role combination for C05/C01; compare browsers, cloud sync, conferencing, updaters, P2P, approved bandwidth sharing, embedded SDKs and quiet/intermittent relays.
- C05: identified execution supports software-policy review; consent and malicious bundles need separate evidence. Preserve catalogue/approval conflicts, exact-versus-metadata history, created-versus-parent identity and event-time account scope.
- C01: role-valid contact remains primary. Ordered connections are context; default sequence-first ranking is suspended. Require event-time intel/pair applicability, tuple-wide process consistency and target-sensitive, explicitly noncausal protection association. Missing optional events never erase an anchor.
- C06: keep the paper sparse/benign twins and app-only comparison. All-outcome baseline presence, typed identity lanes, scoped-ID reduction, deterministic rate/rank conventions and distinct-ID/later-event-time success handling are required. No cohort alone establishes coordination, proxy transport or compromise.

The immediate endpoint question is what observed evidence would distinguish an unexpected relay role or deployment from those benign controls without depending entirely on a historical hash, current IOC list or high fan-out. Improve or reject a concrete candidate; do not merely repeat background research or execute later tasks.

Actual customer ingestion/category/hash/key/ID coverage, directory mapping, comparable history, approvals, current control intelligence and task pairs remain unknown/unsupplied. No new collection, proxy-node interaction, runnable module or deployment is authorized. C04 remains reserve; C03/C02 retain their collection limits.

Re-fetch current main and the blob SHA before saving; preserve concurrent changes and stop if L05 has already completed. Execute exactly one next pass.
