...
**L14 correction:** Replace “unchanged/exact binary” in current interpretation with **same observed SHA1** unless independently corroborated by SHA256, file size and created-image certificate evidence. The review unit is one bundled scope + DeviceId + AppPath + AppName + current SHA1 case, not a hostname row. Preserve application-new, C11-only and shared hostname sets and their witnesses inside the case; a mixed case counts once. Compute separate application/build population and exclusion denominators before the matched K=20 comparison. A hit establishes a possible observed audience/role change under the recorded hash, not relay participation, authorization or compromise. Next validation: run S90 first, record scan/runtime cost, then compare independently adjudicated case decisions and review minutes; retire the hash layer if upgrades, reversions, parallel versions, feature flags, CDN changes or missing hashes dominate its additions.
...
**L14 correction:** Remove C13 from the active field shortlist. A shifted placebo is insufficient: A13's benign repeated common timer produced the same zero-lag pulse for delivered, withheld and delayed task conditions while all shifted placebos stayed negative. Reopen only after an authorized instrumented mapping independently identifies control contact, task receipt, proxy readiness and target connection. Bundle one review case by workspace, device, process-start key, controller-intelligence entry and 24-hour window; deduplicate overlapping epochs. Reject task-response causality if fixed local timing explains the pulse, and reject ranking value if it changes no C01 decision or review effort. No such sensor mapping currently exists.
...
**L14 correction:** Demote from “first practical” to an **input-gated governance baseline**. A run is not meaningful until a reviewed catalogue provides a hash algorithm actually populated for the created image and an event-time policy/approval register identifies the effective decision owner. Deduplicate repeated process launches to one deployment/install review episode per scope, device, product, hash/path and bounded period. A hit establishes observed execution of the catalogued artifact plus whatever separate policy fact is joined; it does not establish consent, current forwarding, covert bundling, endpoint compromise or session compromise. Unknown embedded SDKs remain out of scope. Next validation: measure catalogue coverage and adjudicate P01–P11 at the episode level against an exact-hash-only baseline.
...
| H08 | Identifiable proxyware execution and provenance can support software-policy review before forwarding is proven. | narrowed in L14; C05 input-gated governance baseline | S17/S10/S34/S04; P01–P11 paper fixtures separate identity, authorization and malicious-chain evidence. | Same authentic image cannot distinguish consent or hidden bundling. A reviewed compatible-hash catalogue and event-time effective approval register are absent; per-launch review would duplicate one deployment episode. | Verify input coverage, deduplicate to deployment/install episodes, and compare enrichment with an exact-hash-only baseline; do not run as a practical hunt until inputs are decision-complete. |
...
| H17 | Same-observed-SHA1 self-history can add a role/configuration-review queue only for destinations already seen under another or unknown application hash; its field decision value is unknown. | narrowed after L14; C11 implemented case-level experiment, not breakthrough | S88–S90 implement the comparison; A12 proves the subset relation; A13 shows why hostname rows cannot be the review budget. | SHA1 is not collision-resistant identity; SHA256 may be absent. C11-only is enriched for upgrades, reversions, parallel versions, feature flags, CDN changes and missing-hash history. The current coverage output does not by itself provide both whole-population denominators. | Bundle by scope/device/path/name/current SHA1; retain all host witnesses; inventory application/build populations and exclusions; compare K=20 independently adjudicated decisions/time. Retire if version artefacts dominate or no decision benefit appears. |
...
| H24 | Repeated short-lag control-to-noncontrol uplift may prioritize C01 leads only if independently mapped task/proxy-readiness/target events respond to delivered, withheld and delayed tasks beyond a fixed common timer. | weakened in L14; removed from active field shortlist, sensor/lab-gated | S85 supplies a polling/task mechanism and S84 possible fields; A11 checks only the reducer. A13 shows a task-driven world responds to intervention while a common-timer twin does not. | Persistent sockets/event suppression can erase epochs. Updaters and approved forwarders are twins. A13 proves a shifted placebo can be negative while a benign timer exactly reproduces the focal pulse. | Reopen only after independent event mapping; hold process/network/local work fixed across delivered/withheld/delayed tasks. Reject causality on timer equivalence and ranking on no changed C01 decision or review effort. |
...
| S91 | [Microsoft Edge Update policies](https://learn.microsoft.com/en-us/deployedge/microsoft-edge-update-policies) | Accessed 2026-09-09 | Official policy documentation describes periodic update checks and applying available updates, supporting a benign poll-then-work common-cause mechanism. | It does not establish that Edge produces the exact C13 DNE sequence or controller role. |
| S92 | [Windows Autopatch: Microsoft Edge management](https://learn.microsoft.com/en-us/windows/deployment/windows-autopatch/manage/windows-autopatch-edge) | Accessed 2026-09-09 | Official documentation gives recurring Edge check/update cadences, supporting ordinary version churn and repeated timers as adverse controls. | Product cadence is an example, not a tenant observation or proof about any candidate process. |
| S93 | [DeviceProcessEvents schema](https://learn.microsoft.com/en-us/defender-xdr/advanced-hunting-deviceprocessevents-table) | Updated 2026-08-07; accessed 2026-09-09 | Created-image SHA1, SHA256, size and version fields exist; SHA256 is usually unpopulated. Initiating-process signing fields describe the initiator; created-image signing requires a SHA1 join to DeviceFileCertificateInfo. | Documents schema, not workspace population or cryptographic uniqueness. |
| S94 | [NIST retires SHA-1](https://www.nist.gov/news-events/news/2022/12/nist-retires-sha-1-cryptographic-algorithm) | Published 2022-12-15; updated 2026-04-08; accessed 2026-09-09 | NIST recommends migration from SHA-1 because collisions make security reliance inadvisable. | This does not invalidate SHA1 as a practical telemetry grouping key, but blocks calling it exact byte identity without corroboration. |
...
## L14 — One application is one investigation; the timer survives the placebo

**Question.** Do the selected hunts survive an independent adversarial review of their evidence, telemetry, logic, benign controls and review accounting?

**Result.** Not in their L13 order. C11 survives only as a corrected field-value experiment; C05 is conditional on decision-complete catalogue/approval inputs; C01+C13 leaves the active field shortlist until a sensor can expose the causal events; C10 is rejected for ordinary Sentinel telemetry. The breakthrough slate remains empty.

A separate model reviewer read the latest research file, merged C11 module and primary schema/sensor sources before reading the planned response. This is useful independence of reasoning, not human peer review or new empirical evidence. Its strongest objections were then encoded in A13 rather than accepted by authority.

### Adversarial decisions

| Candidate | Strongest objection | Class | Resolution |
|---|---|---|---|
| C05 | The one saved SHA256 seed may not be populated in DeviceProcessEvents; no reviewed catalogue or effective approval register exists. Repeated process launches would also overcount one deployment. | Fatal to a current run; repairable design | Demote to input-gated governance. Require compatible exact hashes plus event-time approval and deduplicate to deployment/install episodes. |
| C11 | S89 emits hostname evidence rows, so K=20 rows can be consumed by one high-fanout benign application. The current coverage output does not directly expose both app-wide and build-wide population denominators. | Fatal evaluation unit; repairable logic. Denominator/external validity unresolved | Bundle one device/application/current-SHA1 case with all host sets and witnesses; compute separate populations/exclusions. |
| C11 identity | SHA1 grouping and same path/name do not prove byte identity, configuration or purpose; ordinary updates, reversions, parallel versions, feature flags, CDN changes and missing hashes preferentially create C11-only cases. | Repairable wording; unresolved field value | Say “same observed SHA1”; add SHA256/size/created-image certificate conflict context where populated. Kill the layer on version artefacts or no decision/time benefit. |
| C01+C13 | DNE records connections, not task receipt. Persistent sockets may erase epochs, while an approved pull worker or repeated local timer can reproduce the pulse. | Fatal to present field execution; repairable only with instrumentation | Remove from active field ranking. Require independent event mapping and delivered/withheld/delayed intervention; shifted placebos alone do not suffice. |
| C10 | DNE has no TLS fingerprint and Suricata flow_id correlates Suricata records, not an endpoint process. | Fatal for ordinary Sentinel; instrumentation question unresolved | Reject as an ordinary Sentinel hunt. Retain only a gated study where an existing pre-interception TLS source has unique socket/process attribution. |

### A13 — review-unit and intervention counterexamples

A13 is a deterministic synthetic accounting/intervention test, not KQL or sensor validation.

| Challenge | Constructed result | Consequence |
|---|---|---|
| Equal K=20 **cases** | Application history: 20/20 useful-role-review cases. C11: 10/20 because ten benign version-churn cases displaced ten application-new cases. | More C11 cases is not benefit; compare changed decisions and review time. |
| K=20 **hostname rows** | All 20 selected C11 rows came from one benign high-fanout case; zero useful rows. The panel had 41 C11-eligible cases and 80 rows. | Hostnames are evidence items inside a case, never independent review slots. |
| Mixed case | One case contained one application-new plus ten C11-only hosts. | Count it once, preserve its host sets and witnesses, and do not let multiplicity improve rank. |
| Task-driven timer | Delivered pulse 1.0; withheld 0; delayed 0 in the original window. | Intervention can distinguish this constructed mechanism. |
| Benign common timer | Delivered, withheld and delayed all pulse 1.0; every +30-second shifted placebo was 0. | A negative shifted placebo does not reject a common timer; C13 needs task intervention and independent mapping. |

Twenty assertions passed twice with byte-identical output. Script SHA-256 is 73bb9adfed90424ad28f153666a9097ccf4a7897f4d6d459d4de8601a5ec10e5; output SHA-256 is dea076496c8f9675947b31370b25999e54fe09f70d23fd8c8f39d23da46dd6fc. The constructed labels deliberately make version churn adverse; they estimate neither prevalence nor precision.

### Corrected field protocols

**C11.** Run S90 coverage first on one bounded scope, cohort, action lane and frozen 14-day/7-day split. Build explicit app-population and current-build-population tables and count missing/invalid identity, missing SHA1, no current-build history, sparse named days and absent candidate hostnames before matched filtering. Use a case key of scope, DeviceId, AppPath, AppName and current observed SHA1. Store application-new, C11-only, shared and unchanged host sets plus first/last witnesses inside the case. Give both methods K=20 cases under the same method-independent order and review their union once. Record query rows/bytes/duration/throttling, analyst minutes, decision and displaced cases. Keep only if independent role/configuration decisions improve or equivalent decisions cost materially less.

**C05.** First prove a reviewed catalogue has a hash type populated for created images and that policy records identify the effective approval at event time. Return one deployment/install episode rather than one row per process start; keep separate artifact identity, authorization and malicious-chain evidence. Next compare the full enrichment with an exact-hash execution baseline at equal episode budget.

**C01+C13.** No field query should be built yet. In an authorized instrumented environment, hold process, network, polling and local work fixed; deliver, withhold and delay tasks while mapping task receipt, proxy readiness and target connection independently. One review unit is workspace, device, process-start key, controller-intelligence entry and 24 hours, with overlapping control epochs deduplicated. A fixed timer rejects task-response causality; no changed C01 decision/review effort rejects ranking value.

**C10 reconsideration.** The relay mechanism remains plausible and prior research obtained transport fingerprints with dedicated capture, but ordinary DNE lacks the fingerprint and Suricata's documented flow_id does not supply the Defender process key. This is not an actionable Sentinel candidate. Reopen only if an existing pre-NAT/on-host sensor provides a tested unique tuple/time/process mapping; otherwise keep rejected.

### Interpretation and change of view

A C11 hit is evidence of an observed destination-audience change under one recorded SHA1, not exact code identity, proxy participation, unauthorized use, surrounding-network exposure, endpoint compromise or session compromise. A C05 hit can support a software-policy decision only through separate approval facts. C01 contact plus a pulse remains contact/timing evidence, not remote authorship. C10 remains an instrumentation dependency, not a query awaiting syntax.

The useful-hunt objective still has conditional candidates, but the distinctive-breakthrough objective has not been met. The strongest contribution is measurement discipline: compare decisions at the investigation-case level, expose missing-data denominators, and require causal interventions where timing can be generated by a common scheduler.

### L15 handoff

Produce the final ranked hunt/detection brief by **verified input availability**:

1. If DeviceNetworkEvents coverage is adequate, run S90 then the corrected bundled C11 back-test; do not rank hostname rows.
2. If DeviceProcessEvents plus a reviewed compatible-hash catalogue and effective approval register exist, run C05 at deployment-episode level.
3. If Microsoft Graph linkage exists, retain C04-G as the identity/session reserve.
4. Keep C01+C13 out of the field queue until instrumented mapping/intervention succeeds; keep C10 rejected for ordinary Sentinel.

State separately whether the programme produced useful work and whether it produced a distinctive contribution. Preserve the breakthrough slate as empty unless L15 finds evidence already present in the record. No production promotion follows from the brief.
